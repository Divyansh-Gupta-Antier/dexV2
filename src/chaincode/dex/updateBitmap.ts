...
import { DexPositionData, GetBitMapResDto, GetPoolDto, Pool, UpdatePoolBitmapDto } from "../../api";
import { flipTick, sqrtPriceToTick } from "../../api/utils/dex/tick.helper";
...
export async function makeBitMapChanges(ctx: GalaChainContext, dto: UpdatePoolBitmapDto): Promise<Pool> {
  ...
  pool.liquidity = new BigNumber(expectedLiquidity); //Update pool liquidity if its incorrect
  pool.bitmap = bitMap;
...
async function getPoolChanges(ctx: GalaChainContext, pool: Pool): Promise<GetBitMapResDto> {
  ...
  return { bitMap, expectedLiquidity: expectedLiquidity.toString(), liquidity: pool.liquidity.toString() };
}
